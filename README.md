...
example. In particular, it uses columns `score`, `weight`, `label`, and
`category` to create the plot.
...
petals with discrete colors determined by the `label` values, in which
case you will likely want to provide a `colors` palette with at least as
