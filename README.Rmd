...
it uses columns `score`, `weight`, `name_flower`, and `name_supra` to create the plot.
...
from 0 to 1. The petal labels are taken from the `name_flower` variable, and the 
grouping category labels are taken from the `name_supra` variable. Other
columsn in the data frame are ignored.
...
with discrete colors determined by the `name_flower` categories, in which case 
...
which we also provide only missing values to `name_supra` so that no grouping 
labels are used:
